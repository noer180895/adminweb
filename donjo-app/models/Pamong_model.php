<?php class Pamong_model extends CI_Model{
	...
	function list_data($user){
		...
		// get data id_desa from user login
		$intUser = (int)$user;
		$ff = $this->db->select('id_desa,id_grup');
		$ff = $this->db->from('user');
		$ff = $this->db->where('id', $intUser);
		$q = $this->db->get(); 
		$list = $q->row_array();
		$intDesa = (int)$list['id_desa'];
		$intGrup = (int)$list['id_grup']; 
		...
		if($intGrup == 1){
			...
			$sql   = "SELECT u.* FROM tweb_desa_pamong u WHERE id_desa = ".$intDesa."";
			$sql .= $this->search_sql();
			$sql .= $this->filter_sql();
			$query = $this->db->query($sql);
			$data  = $query->result_array();
	...
	function insert(){
		...
		$sql = "INSERT INTO tweb_desa_pamong (id_desa,pamong_nama,pamong_nip,pamong_nik,jabatan,pamong_status,pamong_tgl_terdaftar,foto)
				VALUES (?,?,?,?,?,?,?,?)";

		$now = date('Y-m-d');			
		$outp = $this->db->query($sql, array($idDesaInt,$nama,$nip,$nik,$jabatan,$status,$now,$nama_file));
