<?php class Config_model extends CI_Model{
	...
	// for get all data config
	// function get_data_all(){
	// 	$query = $this->db->select('*')->from('config')->get()->result_array();
	// 	return $query;
	// }


	function get_data_all(){
		$sql   = "SELECT u.* FROM config u";
		$sql .= $this->search_sql();
	...
	function get_data_by_desa($userData){
		...
		// kondisi kalau admin maka munculkan semua list desa 
		if($intGroup == 1){
			// var_dump('xx');
			$this->db->select('*');
			$this->db->from('config');
	...
	function search_sql(){
		...
		$cari = $_SESSION['caridesa'];
			$kw = $this->db->escape_like_str($cari);
			$kw = '%' .$kw. '%';
			$search_sql= " AND (u.nama_desa LIKE '$kw' OR u.nip_kepala_desa LIKE '$kw' )";
			return $search_sql;
			}
		}
	...
	function insert(){
		...
		// // die();
		unset($data['old_logo']);	
		$output = $this->db->insert('config',$data);
