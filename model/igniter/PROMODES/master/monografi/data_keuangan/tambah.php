...
if(isset($_POST['simpan'])){
    ...
    {
        $s0 = mysql_query("select * from tbinput_monografi where idkat = '5'");
        $d = mysql_fetch_array($s0);
        $idunik = $d['idunik']+1;
        $s1 = mysql_query("select * from tbmonografi_keuangan order by id asc");
        while($d1 = mysql_fetch_array($s1)){
            if($d1['tipe'] == "sub2"){
                $s = mysql_query("select * from tbmonografi_keuangan where id = '$d1[idsub]'");
            ...
            $keuangan = $_POST['keuangan'.$d1['id']];
            $q = mysql_query("insert into tbinput_monografi(idkat,idunik,idsub,idmain,idsub2,nilai,bulan,tahun) values('5','$idunik','$d1[id]','$idsub','$idsub2','$keuangan','$bulan','$tahun')");
        }
        if($q){
            echo "<script>window.location='$server/monografi/data_keuangan/';</script>";
        }
    }
}
?>
    ...
    $tahun.change(function(){
        if($tahun.val() != '' && $bulan.val() != ''){
            $.ajax({
                url:'cektahun.php',
                type:'GET',
                data:{idkat:5,mode:'input',bulan:$bulan.val(),bulan2:0,tahun:$tahun.val(),tahun2:0},
...
while($d1 = mysql_fetch_array($s1)){
    echo "<tr>
            <td>$n1</td>
            <td>$d1[nama]</td>
            <td><input type='text' name='keuangan$d1[id]'></td>
        </tr>";
    $urut = $d1['pengurutan'];
    if($d1['pengurutan'] != '-'){
            $titik = ".";
    ...
    while($d2 = mysql_fetch_array($s2)){
        echo "<tr>
            ...
            <td><input type='text' name='keuangan$d2[id]'></td>";
        if($urut != '-'){
                $urut++;
        }
        $urut2 = $d2['pengurutan'];
        ...
        while($d3 = mysql_fetch_array($s3)){
            echo "<tr>
                ...
                <td><input type='text' name='keuangan$d3[id]'></td>";
            if($urut2 != '-'){
                    $urut2++;
            }
        }
