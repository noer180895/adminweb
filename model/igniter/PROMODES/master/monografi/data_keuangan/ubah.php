...
if(isset($_POST['simpan'])){
    ...
    if($bulan != $bulan2 && $tahun != $tahun2){
        $s = mysql_query("select * from tbinput_monografi where idkat = '5' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            $bulan = $bulan2;
            $tahun = $tahun2;
            $s1 = mysql_query("select * from tbmonografi_keuangan order by posisi asc");
            while($d1 = mysql_fetch_array($s1)){
                $dasar = $_POST['dasar'.$d1['id']];
                $q = mysql_query("update tbinput_monografi set nilai = '$dasar',bulan = '$bulan',tahun='$tahun' where idkat = '5' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
    ...
    $tahun.change(function(){
        if($bulan.val() != '' && $tahun.val() != ''){
            $.ajax({
                url:'cektahun.php',
                type:'GET',
                data:{idkat:5,mode:'ubah',bulan:<?php echo $bulan; ?>,bulan2:$bulan.val(),tahun:<?php echo $tahun; ?>,tahun2:$tahun.val()},
