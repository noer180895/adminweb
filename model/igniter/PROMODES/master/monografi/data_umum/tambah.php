...
if(isset($_POST['simpan'])){
    ...
    else
    {
        ...
        $idunik = $d['idunik']+1;
        $s1 = mysql_query("select * from tbmonografi_umum order by id asc");
        while($d1 = mysql_fetch_array($s1)){
            if($d1['tipe'] == "sub2"){
                $s = mysql_query("select * from tbmonografi_umum where id = '$d1[idsub]'");
            ...
            $q = mysql_query("insert into tbinput_monografi(idkat,idunik,idsub,idmain,idsub2,nilai,bulan,tahun) values('2','$idunik','$d1[id]','$idsub','$idsub2','$umum','$bulan','$tahun')");
        }
        if($q){
...
while($d1 = mysql_fetch_array($s1)){
    echo "<tr>
            <td>$n1</td>
            <td>$d1[nama]</td>
            <td><input type='text' name='umum$d1[id]'></td>
        </tr>";
    $urut = $d1['pengurutan'];
    if($d1['pengurutan'] != '-'){
            $titik = ".";
    ...
    while($d2 = mysql_fetch_array($s2)){
        echo "<tr>
            ...
            <td><input type='text' name='umum$d2[id]'></td>";
        if($urut != '-'){
                $urut++;
        }
        $urut2 = $d2['pengurutan'];
        ...
        while($d3 = mysql_fetch_array($s3)){
            echo "<tr>
                ...
                <td><input type='text' name='umum$d3[id]'></td>";
            if($urut2 != '-'){
                    $urut2++;
            }
        }
