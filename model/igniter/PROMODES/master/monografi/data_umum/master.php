...
if($mod == "tambah"){
    ...
    if($tipe == "main"){
        if(isset($_POST['simpan'])){
            ...
            $s = mysql_query("select max(posisi) as urut from tbmonografi_umum where tipe='main'");
            $d = mysql_fetch_array($s);
            $posisi  = $d['urut']+1;

            for($i = 0;$i < sizeof($nama);$i++){
               if($nama[$i] != ""){
                   $q = mysql_query("insert into tbmonografi_umum(idsub,tipe,nama,pengurutan,posisi) values('0','main','".mysql_real_escape_string($nama[$i])."','".$pengurutan[$i]."','".($posisi+$i)."')");
               }
            }

            if($q){
                echo "<script>window.location='$server/monografi/lain-lain/data_umum/';</script>";
            }
        } 
?>
...
<?php
        ...
        if(isset($_POST['simpan'])){
            ...
            $s = mysql_query("select max(posisi) as urut from tbmonografi_umum where tipe='sub' and idsub='$idsub'");
            $d = mysql_fetch_array($s);
            $posisi  = $d['urut']+1;

            for($i = 0;$i < sizeof($nama);$i++){
               if($nama[$i] != ""){
                   $q = mysql_query("insert into tbmonografi_umum(idsub,tipe,nama,pengurutan,posisi) values('$idsub','sub','".mysql_real_escape_string($nama[$i])."','".$pengurutan[$i]."','".($posisi+$i)."')");
               }
            }
...
<?php
        ...
        if(isset($_POST['simpan'])){
            ...
            $s = mysql_query("select max(posisi) as urut from tbmonografi_umum where tipe='sub2'  and idsub='$idsub'");
            $d = mysql_fetch_array($s);
            $posisi  = $d['urut']+1;

            for($i = 0;$i < sizeof($nama);$i++){
               if($nama[$i] != ""){
                   $q = mysql_query("insert into tbmonografi_umum(idsub,tipe,nama,posisi) values('$idsub','sub2','".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
               }
            }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbmonografi_umum where id='$id'");
    ...
    if($tipe == "main"){
        if(isset($_POST['simpan'])){
            if(isset($_POST['simpan'])){
                ...
                $q = mysql_query("update tbmonografi_umum set nama = '$nama', pengurutan = '$pengurutan' where id = '$id'");
                if($q){
                    echo "<script>window.location='$server/monografi/lain-lain/data_umum/';</script>";
                }
            }
...
<?php
        ...
        if(isset($_POST['simpan'])){
            ...
            if($data != $d['idsub']){
                $s = mysql_query("select max(posisi) as urut from tbmonografi_umum where idsub = '$data'");
                $d = mysql_fetch_array($s);
                $posisi  = $d['urut']+1;
            }
            else
            ...
            $q = mysql_query("update tbmonografi_umum set nama = '$nama',
                                                            idsub = '$data',
                                                            pengurutan = '$pengurutan',
                                                            posisi = '$posisi'
                                                            where id = '$id'");
            $s2 = mysql_query("select * from tbmonografi_umum where tipe='sub2' and idsub = '$id'");
    ...
    <table>
        <tr>
            ...
            <td><select name='data' required>
                    <?php
                        $s = mysql_query("select * from tbmonografi_umum where tipe='main'");
...
<form method="post">
    <table>
        <tr>
            ...
            <td><select name='data' required>
                    <?php
                        $s = mysql_query("select * from tbmonografi_umum where tipe='sub'");
                        while($dd = mysql_fetch_array($s)){
                            if($dd[id] == $idsub){
                                echo "<option value='$dd[id]' selected>$dd[nama]</option>";
                            }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbmonografi_umum where id='$id'");
    $q = mysql_query("delete from tbmonografi_umum where idsub='$id'");
    if($q){
        echo "<script>window.location='$server/monografi/lain-lain/data_umum/';</script>";
    }
}
