...
if(isset($_POST['simpan'])){
    $tahun = $_POST['tahun'];
    $bulan = $_POST['bulan'];
    $s = mysql_query("select * from tbinput_monografi where idkat = '7' and bulan = '$bulan' and tahun ='$tahun'");
    $r = mysql_num_rows($s);
    if($r != 0){
    ...
    {
        $s0 = mysql_query("select * from tbinput_monografi where idkat = '7'");
        $d = mysql_fetch_array($s0);
        $idunik = $d['idunik']+1;
        $s1 = mysql_query("select * from tbmonografi_trantib order by id asc");
        while($d1 = mysql_fetch_array($s1)){
            if($d1['tipe'] == "sub2"){
                $s = mysql_query("select * from tbmonografi_trantib where id = '$d1[idsub]'");
            ...
            $q = mysql_query("insert into tbinput_monografi(idkat,idunik,idsub,idmain,idsub2,nilai,bulan,tahun) values('7','$idunik','$d1[id]','$idsub','$idsub2','$trantib','$bulan','$tahun')");
        }
        if($q){
    ...
    $tahun.change(function(){
        if($tahun.val() != '' && $bulan.val() != ''){
            $.ajax({
                url:'cektahun.php',
                type:'GET',
                data:{idkat:7,mode:'input',bulan:$bulan.val(),bulan2:0,tahun:$tahun.val(),tahun2:0},
...
while($d1 = mysql_fetch_array($s1)){
    echo "<tr>
            <td>$n1</td>
            <td>$d1[nama]</td>
            <td><input type='text' name='trantib$d1[id]'></td>
        </tr>";
    $urut = $d1['pengurutan'];
    if($d1['pengurutan'] != '-'){
            $titik = ".";
    ...
    while($d2 = mysql_fetch_array($s2)){
        echo "<tr>
            ...
            <td><input type='text' name='trantib$d2[id]'></td>";
        if($urut != '-'){
                $urut++;
        }
        $urut2 = $d2['pengurutan'];
        ...
        while($d3 = mysql_fetch_array($s3)){
            echo "<tr>
                ...
                <td><input type='text' name='trantib$d3[id]'></td>";
            if($urut2 != '-'){
                    $urut2++;
            }
        }
