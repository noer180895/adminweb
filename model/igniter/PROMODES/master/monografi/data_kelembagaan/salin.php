...
if(isset($_POST['salin'])){
    ...
    $c = mysql_query("select * from tbinput_monografi where idkat = '6' and bulan = '$bulan2' and tahun ='$tahun2'");
    $r = mysql_num_rows($c);
    if($r != 0){
        ...
        while($d = mysql_fetch_array($s)){
            $q = mysql_query("insert into tbinput_monografi(idkat,idunik,idsub,idmain,idsub2,nilai,bulan,tahun) values('6','$idunik','$d[idsub]','$d[idmain]','$d[idsub2]','$d[nilai]','$bulan2','$tahun2')");
        }
        if($q){
            echo "<script>alert('Data berhasil disalin !');window.location='$server/monografi/data_kelembagaan/';</script>";
        }
        else{
            echo "<script>alert('Gagal menyalin data !, pastikan data tersedia pada bulan yang ingin disalin.');window.location='$server/monografi/data_kelembagaan/';</script>";
        }
    }
}
