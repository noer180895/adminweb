...
if(isset($_POST['simpan'])){
    ...
    if($bulan != $bulan2 && $tahun != $tahun2){
        $s = mysql_query("select * from tbinput_monografi where idkat = '6' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            $bulan = $bulan2;
            $tahun = $tahun2;
            $s1 = mysql_query("select * from tbmonografi_kelembagaan order by posisi asc");
            while($d1 = mysql_fetch_array($s1)){
                $kelembagaan = $_POST['kelembagaan'.$d1['id']];
                $q = mysql_query("update tbinput_monografi set nilai = '$kelembagaan',bulan = '$bulan',tahun='$tahun' where idkat = '6' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
    ...
    $tahun.change(function(){
        if($bulan.val() != '' && $tahun.val() != ''){
            $.ajax({
                url:'cektahun.php',
                type:'GET',
                data:{idkat:6,mode:'ubah',bulan:<?php echo $bulan; ?>,bulan2:$bulan.val(),tahun:<?php echo $tahun; ?>,tahun2:$tahun.val()},
...
while($d1 = mysql_fetch_array($s1)){
    ...
    if($_r1 != 0){
        echo "<tr>
                <td>$n1</td>
                <td>$d1[nama]</td>
                <td><input type='text' name='kelembagaan$d1[id]' value='$_d1[nilai]'></td>
            </tr>";
        $urut = $d1['pengurutan'];
        if($d1['pengurutan'] != '-'){
                $titik = ".";
        ...
        while($d2 = mysql_fetch_array($s2)){
            ...
            if($_r2 != 0){
                echo "<tr>
                    ...
                    <td><input type='text' name='kelembagaan$d2[id]' value='$_d2[nilai]'></td>";
                if($urut != '-'){
                        $urut++;
                }
                $urut2 = $d2['pengurutan'];
                ...
                while($d3 = mysql_fetch_array($s3)){
                    ...
                    if($_r3 != 0){
                        echo "<tr>
                            ...
                            <td><input type='text' name='kelembagaan$d3[id]' value='$_d3[nilai]'></td>";
                        if($urut2 != '-'){
                                $urut2++;
                        }
                    }
