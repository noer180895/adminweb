...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbkewarganegaraan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbkewarganegaraan(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/kewarganegaraan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbkewarganegaraan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbkewarganegaraan set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/kewarganegaraan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbkewarganegaraan where id='$id'");
    if($q){
        echo "<script>window.location='$server/psdm/lain-lain/kewarganegaraan/';</script>";
    }
}
else
{
    ...
    $n1 = 1;
    $s1= mysql_query("select * from tbkewarganegaraan order by posisi asc");
