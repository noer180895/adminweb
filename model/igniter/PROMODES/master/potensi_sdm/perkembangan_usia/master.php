...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbperkembangan_usia");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbperkembangan_usia(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/perkembangan_usia/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbperkembangan_usia where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbperkembangan_usia set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/perkembangan_usia/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbperkembangan_usia where id='$id'");
    if($q){
        echo "<script>window.location='$server/psdm/lain-lain/perkembangan_usia/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>PERKEMBANGAN USIA</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
