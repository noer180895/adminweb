...
echo "<h2>CACAT MENTAL DAN FISIK</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbcacat_mental where tipe = '$tipe'");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbcacat_mental(nama,tipe,posisi) values('".mysql_real_escape_string($nama[$i])."','$tipe','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/cacat_metal/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbcacat_mental where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbcacat_mental set nama = '$nama',
                                                            tipe = '$tipe',
                                                            posisi = '$posisi'
                                                            where id = '$id'");
        if($q){
...
<form method="post">
    <table>
        ...
        <tr>
            ...
            <td><select name='tipe' required>
                    <option value=''>- Pilih Tipe</option>
                    <option value='1' <?php if($tipe == 1) echo "selected";?>>Fisik</option>
                    <option value='2' <?php if($tipe == 2) echo "selected";?>>Mental</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbcacat_mental where id='$id'");
    if($q){
        echo "<script>window.location='$server/psdm/lain-lain/cacat_metal/';</script>";
    }
}
