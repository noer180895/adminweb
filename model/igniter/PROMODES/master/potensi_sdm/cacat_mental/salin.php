...
if(isset($_POST['salin'])){
    ...
    $c = mysql_query("select * from tbinput_sdm where idkat = '8' and tahun ='$tahun2'");
    $r = mysql_num_rows($c);
    if($r != 0){
        ...
        while($d = mysql_fetch_array($s)){
            $q = mysql_query("insert into tbinput_sdm(idkat,idunik,idsub,tipe,nilai,tahun) values('8','$idunik','$d[idsub]','$d[tipe]','$d[nilai]','$tahun2')");
        }
        if($q){
            echo "<script>alert('Data berhasil disalin !');window.location='$server/psdm/cacat_mental/';</script>";
        }
        else{
            echo "<script>alert('Gagal menyalin data !, pastikan data tersedia pada tahun yang ingin disalin.');window.location='$server/psdm/cacat_mental/';</script>";
        }
    }
}
