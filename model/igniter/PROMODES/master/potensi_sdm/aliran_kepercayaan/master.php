...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbaliran_kepercayaan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbaliran_kepercayaan(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/aliran_kepercayaan/';</script>";
        }
    }
...
<form method="post">
    <table>
        <tr>
            <td>Agama/Aliran Kepercayaan</td>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbaliran_kepercayaan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbaliran_kepercayaan set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/aliran_kepercayaan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbaliran_kepercayaan where id='$id'");
    if($q){
        echo "<script>window.location='$server/psdm/lain-lain/aliran_kepercayaan/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>AGAMA/ALIRAN KEPERCAYAAN</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
