...
echo "<h2>KUALITAS ANGKATAN KERJA</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbkualitas_angker");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbkualitas_angker(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/kualitas_angker/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbkualitas_angker where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbkualitas_angker set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psdm/lain-lain/kualitas_angker/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbkualitas_angker where id='$id'");
    if($q){
        echo "<script>window.location='$server/psdm/lain-lain/kualitas_angker/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>ANGKATAN KERJA</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
