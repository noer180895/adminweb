...
if(isset($_POST['simpan'])){
    $tahun2 = $_POST['tahun'];
    if($tahun != $tahun2){
        $s = mysql_query("select * from tbinput_sdm where idkat = '3' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            ...
            while($d1 = mysql_fetch_array($s1)){
                ...
                $q = mysql_query("update tbinput_sdm set nilai = '$val',tahun='$tahun' where idkat = '3' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
                echo "<script>window.location='$server/psdm/tingkat_pendidikan/';</script>";
            }
        }
    }
    else
...
echo "<form method='post'>
    ...
    <table border='1'>
        <tr>
            <th colspan='3'>TINGKAT PENDIDIKAN</th>
        </tr>
        <tr>
...
$s1 = mysql_query("select * from tbtingkat_pendidikan order by posisi asc");
while($d1 = mysql_fetch_array($s1)){
    $_s1 = mysql_query("select * from tbinput_sdm where idkat = '3' and idsub = '$d1[id]' and tahun = '$tahun'");
