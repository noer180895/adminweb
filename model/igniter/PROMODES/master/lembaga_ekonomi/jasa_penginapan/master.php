...
echo "<h2>USAHA JASA PENGINAPAN</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbjasa_penginapan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbjasa_penginapan(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/lembaga_ekonomi/lain-lain/jasa_penginapan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbjasa_penginapan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbjasa_penginapan set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/lembaga_ekonomi/lain-lain/jasa_penginapan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbjasa_penginapan where id='$id'");
    if($q){
        echo "<script>window.location='$server/lembaga_ekonomi/lain-lain/jasa_penginapan/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>USAHA JASA PENGINAPAN</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
