...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tblembaga_ekonomi where tipe = '$tipe'");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tblembaga_ekonomi(nama,tipe,posisi) values('".mysql_real_escape_string($nama[$i])."','$tipe','".($posisi+$i)."')");
           }
        }
        if($q){
            echo "<script>window.location='$server/lembaga_ekonomi/lain-lain/lembaga_ekonomi/';</script>";
        }
    }
    ...
    <table>
        <tr>
            ...
            <td> : <select name='tipe' required>
                    ...
                    <option value='1'>Unit Usaha Desa</option>
                    <option value='2'>Jasa Lembaga Keuangan</option>
                    <option value='3'>Industri Kecil dan Menengah</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tblembaga_ekonomi where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tblembaga_ekonomi set nama = '$nama',
                                                            tipe = '$tipe',
                                                            posisi = '$posisi'
                                                            where id = '$id'");
        if($q){
...
<form method="post">
    <table>
        ...
        <tr>
            ...
            <td><select name='tipe' required>
                    <option value=''>- Pilih Tipe</option>
                    <option value='1' <?php if($tipe == 1) echo "selected";?>>Unit Usaha Desa</option>
                    <option value='2' <?php if($tipe == 2) echo "selected";?>>Jasa Lembaga Keuangan</option>
                    <option value='3' <?php if($tipe == 3) echo "selected";?>>Industri Kecil dan Menengah</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tblembaga_ekonomi where id='$id'");
    if($q){
        echo "<script>window.location='$server/lembaga_ekonomi/lain-lain/lembaga_ekonomi/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th colspan='3'>LEMBAGA EKONOMI</th>
            </tr>    
            <tr>
                <th>#</th>
                <th align=left>LEMBAGA EKONOMI DAN UNIT USAHA DESA/KELURAHAN</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
    ...
    echo "<tr>
            <th>#</th>
            <th align=left>JASA LEMBAGA KEUANGAN</th>
            <th>Pilihan</th>
        </tr>";
    $n2 = 1;
    ...
    echo "<tr>
            <th>#</th>
            <th align=left>INDUSTRI KECIL DAN MENENGAH</th>
            <th>Pilihan</th>
        </tr>";
    $n3 = 1;
