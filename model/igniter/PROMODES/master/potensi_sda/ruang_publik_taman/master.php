...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbruang_publik");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbruang_publik(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psda/lain-lain/ruang_publik_taman/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbruang_publik where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbruang_publik set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psda/lain-lain/ruang_publik_taman/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbruang_publik where id='$id'");
    if($q){
        echo "<script>window.location='$server/psda/lain-lain/ruang_publik_taman/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>RUANG PUBLIK/TAMAN</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
