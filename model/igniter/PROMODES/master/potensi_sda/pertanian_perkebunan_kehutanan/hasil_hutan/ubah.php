...
if(isset($_POST['simpan'])){
    $tahun2 = $_POST['tahun'];
    if($tahun != $tahun2){
        $s = mysql_query("select * from tbinput_p2k where idkat = '4' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            ...
            while($d1 = mysql_fetch_array($s1)){
                ...
                $q = mysql_query("update tbinput_p2k set nilai = '$val',tahun='$tahun' where idkat = '4' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
...
$s1 = mysql_query("select * from tbhasil_hutan order by posisi asc");
while($d1 = mysql_fetch_array($s1)){
    $_s1 = mysql_query("select * from tbinput_p2k where idkat = '4' and idsub = '$d1[id]' and tahun = '$tahun'");
    ...
    if($_r1 != 0){
        echo "<tr>
                <td>$d1[nama]</td>
                <td><input type='text' name ='hasil$d1[id]' value='$_d1[nilai]'/> $d1[satuan]</td>
            </tr>";
    }
}
echo "<tr>
