...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbhasil_hutan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbhasil_hutan(nama,satuan,posisi) values('".mysql_real_escape_string($nama[$i])."','".$satuan[$i]."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbhasil_hutan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbhasil_hutan set nama = '$nama',satuan = '$satuan' where id = '$id'");
        if($q){
            echo "<script>window. location='$server/psda/pertanian_perkebunan_kehutanan/hasil_hutan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbhasil_hutan where id='$id'");
    if($q){
        echo "<script>window. location='$server/psda/pertanian_perkebunan_kehutanan/hasil_hutan/';</script>";
    }
}
else
{
    ...
    $n1 = 1;
    $s1= mysql_query("select * from tbhasil_hutan order by posisi asc");
