...
echo "<h2>KONDISI HUTAN</h2>";
 
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        $nama = $_POST['nama'];
        
        $s = mysql_query("select max(posisi) as urut from tbkondisi_hutan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbkondisi_hutan(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbkondisi_hutan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbkondisi_hutan set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window. location='$server/psda/pertanian_perkebunan_kehutanan/kondisi_hutan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbkondisi_hutan where id='$id'");
    if($q){
        echo "<script>window. location='$server/psda/pertanian_perkebunan_kehutanan/kondisi_hutan/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>Kondisi Hutan</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
