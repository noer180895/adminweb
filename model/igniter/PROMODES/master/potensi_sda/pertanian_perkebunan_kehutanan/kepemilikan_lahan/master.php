...
echo "<div class='module-head'>KEPEMILIKAN LAHAN</div>";
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        $nama = $_POST['nama'];
        $tipe = $_POST['tipe'];
        
        $s = mysql_query("select max(posisi) as urut from tbkepemilikan_lahan where tipe = '$tipe'");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbkepemilikan_lahan(nama,tipe,posisi) values('".mysql_real_escape_string($nama[$i])."','$tipe','".($posisi+$i)."')");
           }
        }
    ...
    <table>
        <tr>
            ...
            <td> : <select name='tipe' required>
                    ...
                    <option value='1'>Kepemilikan Lahan</option>
                    <option value='2'>Kelurahan yang Memiliki lahan</option>
                    <option value='3'>Luas Lahan</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbkepemilikan_lahan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbkepemilikan_lahan set    nama = '$nama',
                                                            tipe = '$tipe',
                                                            posisi = '$posisi'
                                                            where id = '$id'");
        if($q){
...
<form method="post">
    <table>
        ...
        <tr>
            ...
            <td><select name='tipe' required>
                    <option value=''>- Pilih Tipe -</option>
                    <option value='1' <?php if($tipe == 1) echo "selected";?>>Kepemilikan Lahan</option>
                    <option value='2' <?php if($tipe == 2) echo "selected";?>>Kelurahan yang Memiliki lahan</option>
                    <option value='3' <?php if($tipe == 3) echo "selected";?>>Luas Lahan</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbkepemilikan_lahan where id='$id'");
    if($q){
        echo "<script>window. location='$server/psda/pertanian_perkebunan_kehutanan/kepemilikan_lahan/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>Kepemilikan Lahan</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
    ...
    echo "<tr>
            <th>#</th>
            <th align=left>Kelurahan yang Memiliki Lahan</th>
            <th>Pilihan</th>
        </tr>";
    $n2 = 1;
