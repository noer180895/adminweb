...
echo "<h2>JENIS DAN ALAT PRODUKSI BUDIDAYA IKAN LAUT DAN PAYAU</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbalat_produksi");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbalat_produksi(nama,satuan,posisi) values('".mysql_real_escape_string($nama[$i])."','".$satuan[$i]."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbalat_produksi where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbalat_produksi set nama = '$nama',satuan = '$satuan' where id = '$id'");
        if($q){
            echo "<script>window.location= '$server/psda/peternakan_perikanan/alat_produksi/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbalat_produksi where id='$id'");
    if($q){
        echo "<script>window.location= '$server/psda/peternakan_perikanan/alat_produksi/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>JENIS DAN ALAT PRODUKSI BUDIDAYA IKAN LAUT DAN PAYAU</th>
                <th>Satuan</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
