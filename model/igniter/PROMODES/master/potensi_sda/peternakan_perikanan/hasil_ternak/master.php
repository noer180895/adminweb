...
echo "<h2>PEMILIK USAHA PENGOLAHAN HASIL TERNAK</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbhasil_ternak");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbhasil_ternak(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbhasil_ternak where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbhasil_ternak set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psda/peternakan_perikanan/hasil_ternak/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbhasil_ternak where id='$id'");
    if($q){
        echo "<script>window.location='$server/psda/peternakan_perikanan/hasil_ternak/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>PENGOLAHAN HASIL TERNAK</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
