...
echo "<h2>KETERSEDIAAN HIJAUAN PAKAN TERNAK</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbpakan_ternak");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbpakan_ternak(nama,satuan,posisi) values('".mysql_real_escape_string($nama[$i])."','".$satuan[$i]."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psda/peternakan_perikanan/pakan_ternak/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbpakan_ternak where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbpakan_ternak set nama = '$nama',satuan = '$satuan' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psda/peternakan_perikanan/pakan_ternak/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbpakan_ternak where id='$id'");
    if($q){
        echo "<script>window.location='$server/psda/peternakan_perikanan/pakan_ternak/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>KETERSEDIAAN HIJAUAN PAKAN TERNAK</th>
                <th>Satuan</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
