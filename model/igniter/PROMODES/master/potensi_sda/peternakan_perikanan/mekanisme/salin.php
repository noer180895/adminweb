...
if(isset($_POST['salin'])){
    ...
    $c = mysql_query("select * from tbinput_p2 where idkat = '9' and tahun ='$tahun2'");
    $r = mysql_num_rows($c);
    if($r != 0){
        ...
        while($d = mysql_fetch_array($s)){
            $q = mysql_query("insert into tbinput_p2(idkat,idunik,idsub,nilai,tahun) values('9','$idunik','$d[idsub]','$d[nilai]','$tahun2')");
        }
        if($q){
            echo "<script>alert('Data berhasil disalin !');window.location='$server/psda/peternakan_perikanan/mekanisme/';</script>";
        }
        else{
            echo "<script>alert('Gagal menyalin data !, pastikan data tersedia pada tahun yang ingin disalin.');window.location='$server/psda/peternakan_perikanan/mekanisme/';</script>";
        }
    }
}
