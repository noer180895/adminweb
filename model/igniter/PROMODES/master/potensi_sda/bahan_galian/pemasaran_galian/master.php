...
echo "<h2>PEMASARAAN HASIL GALIAN</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbpemasaran_galian");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbpemasaran_galian(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psda/bahan_galian/pemasaran_galian/ ';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbpemasaran_galian where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbpemasaran_galian set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psda/bahan_galian/pemasaran_galian/ ';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbpemasaran_galian where id='$id'");
    if($q){
        echo "<script>window.location='$server/psda/bahan_galian/pemasaran_galian/ ';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>Pemasaran Hasil Galian</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
