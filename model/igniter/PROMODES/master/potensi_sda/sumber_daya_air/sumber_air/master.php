...
echo "<h2>SUMBER AIR BERSIH DAN KUALITAS AIR MINUM</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbsumber_air");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbsumber_air(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbsumber_air where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbsumber_air set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psda/sumber_daya_air/sumber_air/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbsumber_air where id='$id'");
    if($q){
        echo "<script>window.location='$server/psda/sumber_daya_air/sumber_air/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>SUMBER AIR BERSIH DAN KUALITAS AIR MINUM</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
