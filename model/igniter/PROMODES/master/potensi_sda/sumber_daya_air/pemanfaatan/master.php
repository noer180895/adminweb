...
echo "<h2>PEMANFAATAN DAN KONDISI DANAU/WADUK/SITU</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbpemanfaatan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbpemanfaatan(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/psda/sumber_daya_air/pemanfaatan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbpemanfaatan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbpemanfaatan set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/psda/sumber_daya_air/pemanfaatan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbpemanfaatan where id='$id'");
    if($q){
        echo "<script>window.location='$server/psda/sumber_daya_air/pemanfaatan/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>PEMANFAATAN DAN KONDISI DANAU/WADUK/SITU</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
