...
if(isset($_POST['simpan'])){
    $tahun2 = $_POST['tahun'];
    if($tahun != $tahun2){
        $s = mysql_query("select * from tbinput_sarpras2 where idkat = '13' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            ...
            while($d1 = mysql_fetch_array($s1)){
                ...
                $q = mysql_query("update tbinput_sarpras2 set nilai = '$prasarana',tahun='$tahun' where idkat = '13' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
    ...
    $tahun.change(function(){
        if($tahun.val() != ''){
            $.ajax({
                url:'cektahun.php',
                type:'GET',
                data:{idkat:13,mode:'ubah',tahun:<?php echo $tahun; ?>,tahun2:$tahun.val()},
