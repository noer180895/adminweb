...
if(isset($_POST['simpan'])){
    $tahun2 = $_POST['tahun'];
    if($tahun != $tahun2){
        $s = mysql_query("select * from tbinput_sarpras2 where idkat = '6' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            ...
            while($d1 = mysql_fetch_array($s1)){
                ...
                $q = mysql_query("update tbinput_sarpras2 set nilai = '$val',tahun='$tahun' where idkat = '6' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
...
while($d1 = mysql_fetch_array($s1)){
    ...
    if($_r1 != 0){
        echo "<tr>
                <td>$d1[nama]</td>
                <td align=right>Sewa <input type='text' name ='sewa$d1[id]'  value='$nilai[0]'/> Buah</td>
                <td align=right>Milik Sendiri <input type='text' name ='milik$d1[id]'  value='$nilai[1]'/> Buah</td>
            </tr>";
    }
}
echo "<tr>
