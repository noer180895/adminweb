...
echo "<h2>SARANA TRANSPORTASI DARAT</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbsarana_darat");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbsarana_darat(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbsarana_darat where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbsarana_darat set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/potensi_sarpras/sarpras_transportasi/sarana_darat/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbsarana_darat where id='$id'");
    if($q){
        echo "<script>window.location='$server/potensi_sarpras/sarpras_transportasi/sarana_darat/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>Sarana Transportasi Darat</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
