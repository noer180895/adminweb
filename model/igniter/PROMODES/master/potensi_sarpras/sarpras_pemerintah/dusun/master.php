...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbsarpras_dusun");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbsarpras_dusun(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbsarpras_dusun where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbsarpras_dusun set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/potensi_sarpras/sarpras_pemerintah/dusun/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbsarpras_dusun where id='$id'");
    if($q){
        echo "<script>window.location='$server/potensi_sarpras/sarpras_pemerintah/dusun/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>Prasarana dan Sarana Dusun/ Lingkungan atau Sebutan Lain</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
