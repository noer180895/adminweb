...
echo "<h2>PRASARANA DAN SARANA PEMERINTAHAN DESA/KELURAHAN</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbsarpras_desa where tipe = '$tipe'");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbsarpras_desa(nama,tipe,posisi) values('".mysql_real_escape_string($nama[$i])."','$tipe','".($posisi+$i)."')");
           }
        }
    ...
    <table>
        <tr>
            ...
            <td> : <select name='tipe' required>
                    ...
                    <option value='3'>Administrasi Pemerintahan Desa/Kelurahan</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbsarpras_desa where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbsarpras_desa set nama = '$nama',
                                                            tipe = '$tipe',
                                                            posisi = '$posisi'
                                                            where id = '$id'");
        if($q){
    ...
    <table>
        ...
        <tr>
            ...
            <td><select name='tipe' required>
                    ...
                    <option value='3' <?php if($tipe == 3) echo "selected";?>>Administrasi Pemerintahan Desa/Kelurahan</option>
                </select>
            </td>
        </tr>
        <tr>
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbsarpras_desa where id='$id'");
    if($q){
        echo "<script>window.location='$server/potensi_sarpras/sarpras_pemerintah/desa/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>Prasarana dan Sarana Pemerintah Desa/Kelurahan</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
    ...
    echo "<tr>
            <th>#</th>
            <th align=left>Administrasi Pemerintahan Desa/Kelurahan</th>
            <th>Pilihan</th>
        </tr>";
    $n3 = 1;
