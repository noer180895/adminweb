...
echo "<h2>PRASARANA DAN SARANA KEBERSIHAN</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbsarpras_kebersihan");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbsarpras_kebersihan(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/potensi_sarpras/lain-lain/sarpras_kebersihan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbsarpras_kebersihan where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbsarpras_kebersihan set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/potensi_sarpras/lain-lain/sarpras_kebersihan/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbsarpras_kebersihan where id='$id'");
    if($q){
        echo "<script>window.location='$server/potensi_sarpras/lain-lain/sarpras_kebersihan/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>PRASARANA DAN SARANA KEBERSIHAN</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
