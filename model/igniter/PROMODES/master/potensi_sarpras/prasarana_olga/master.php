...
echo "<h2>PRASARANA OLAH RAGA</h2>";
...
if($mod == "tambah"){
    if(isset($_POST['simpan'])){
        ...
        $s = mysql_query("select max(posisi) as urut from tbprasarana_olga");
        $d = mysql_fetch_array($s);
        $posisi  = $d['urut']+1;
        
        for($i = 0;$i < sizeof($nama);$i++){
           if($nama[$i] != ""){
               $q = mysql_query("insert into tbprasarana_olga(nama,posisi) values('".mysql_real_escape_string($nama[$i])."','".($posisi+$i)."')");
           }
        }
        
        if($q){
            echo "<script>window.location='$server/potensi_sarpras/lain-lain/prasarana_olga/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $c = mysql_query("select * from tbprasarana_olga where id='$id'");
    ...
    if(isset($_POST['simpan'])){
        ...
        $q = mysql_query("update tbprasarana_olga set nama = '$nama' where id = '$id'");
        if($q){
            echo "<script>window.location='$server/potensi_sarpras/lain-lain/prasarana_olga/';</script>";
        }
    }
...
<?php
    ...
    $id = abs((int)$_GET['id']);
    $q = mysql_query("delete from tbprasarana_olga where id='$id'");
    if($q){
        echo "<script>window.location='$server/potensi_sarpras/lain-lain/prasarana_olga/';</script>";
    }
}
else
{
    ...
    echo "<table>
            <tr>
                <th>#</th>
                <th align=left>PRASARANA OLAH RAGA</th>
                <th>Pilihan</th>
            </tr>";
    $n1 = 1;
    $s1= mysql_query("select * from tbprasarana_olga order by posisi asc");
