...
if(isset($_POST['simpan'])){
    $tahun2 = $_POST['tahun'];
    if($tahun != $tahun2){
        $s = mysql_query("select * from tbinput_sarpras2 where idkat = '12' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            ...
            while($d1 = mysql_fetch_array($s1)){
                ...
                $q = mysql_query("update tbinput_sarpras2 set nilai = '$prasarana',tahun='$tahun' where idkat = '12' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            if($q){
    ...
    $tahun.change(function(){
        if($tahun.val() != ''){
            $.ajax({
                url:'cektahun.php',
                type:'GET',
                data:{idkat:12,mode:'ubah',tahun:<?php echo $tahun; ?>,tahun2:$tahun.val()},
...
$s1 = mysql_query("select * from tbprasarana_energi order by posisi asc");
while($d1 = mysql_fetch_array($s1)){
    $_s1 = mysql_query("select * from tbinput_sarpras2 where idkat = '12' and idsub = '$d1[id]' and tahun = '$tahun'");
    $_d1 = mysql_fetch_array($_s1);
    $_r1 = mysql_num_rows($_s1);
    if($_r1 != 0){
        echo "<tr>
                <td>$d1[nama]</td>
                <td><input type='text' name='prasarana$d1[id]' value='$_d1[nilai]'/> $d1[satuan]</td>
            </tr>";
    }
}
echo "<tr>
