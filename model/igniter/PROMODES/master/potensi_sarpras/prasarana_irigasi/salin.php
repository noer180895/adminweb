...
if(isset($_POST['salin'])){
    ...
    $c = mysql_query("select * from tbinput_sarpras2 where idkat = '9' and tahun ='$tahun2'");
    $r = mysql_num_rows($c);
    if($r != 0){
        ...
        while($d = mysql_fetch_array($s)){
            $q = mysql_query("insert into tbinput_sarpras2(idkat,idunik,idsub,idmain,tipe,nilai,tahun) values('9','$idunik','$d[idsub]','$d[idmain]','$d[tipe]','$d[nilai]','$tahun2')");
        }
        if($q){
            echo "<script>alert('Data berhasil disalin !');window.location='$server/potensi_sarpras/prasarana_irigasi/';</script>";
        }
        else{
            echo "<script>alert('Gagal menyalin data !, pastikan data tersedia pada tahun yang ingin disalin.');window.location='$server/potensi_sarpras/prasarana_irigasi/';</script>";
        }
    }
}
