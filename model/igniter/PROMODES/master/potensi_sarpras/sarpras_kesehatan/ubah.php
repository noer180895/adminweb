...
if(isset($_POST['simpan'])){
    $tahun2 = $_POST['tahun'];
    if($tahun != $tahun2){
        $s = mysql_query("select * from tbinput_sarpras where idkat = '5' and tahun ='$tahun2'");
        $r = mysql_num_rows($s);
        if($r != 0){
        ...
        else
        {
            ...
            while($d1 = mysql_fetch_array($s1)){
                ...
                $q = mysql_query("update tbinput_sarpras set nilai = '$prasarana',tahun='$tahun' where idkat = '5' and idunik = '$idunik' and idsub = '$d1[id]'");
            }
            ...
            while($d2 = mysql_fetch_array($s2)){
                ...
                $q = mysql_query("update tbinput_sarpras set nilai = '$sarana',tahun='$tahun' where idkat = '5' and idunik = '$idunik' and idsub = '$d2[id]'");
            }
            if($q){
...
while($d1 = mysql_fetch_array($s1)){
    ...
    if($_r1 != 0){
        echo "<tr>
                <td>$d1[nama]</td>
                <td><input type='text' name='prasarana$d1[id]' value='$_d1[nilai]'/> Unit</td>
            </tr>";
    }
}
echo    "<tr>
...
while($d2 = mysql_fetch_array($s2)){
    ...
    if($_r2 != 0){
        echo "<tr>
                <td>$d2[nama]</td>
                <td><input type='text' name='sarana$d2[id]' value='$_d2[nilai]'/> Orang</td>
            </tr>";
    }
}
echo "<tr>
